...
class PerformanceStore:
        ...
        try:
            if self.pg:
                with self.conn.cursor() as cur:
                    cur.execute(f"SELECT day,payload_json FROM {STATS_TABLE} WHERE day >= %s",((datetime.now(IST).date()-timedelta(days=35)).isoformat(),));self.data={'days':{str(day):json.loads(payload) for day,payload in cur.fetchall()}}
    ...
    def _save_day(self,key):
        ...
        if self.pg:
            with self.conn.cursor() as cur:cur.execute(f'''INSERT INTO {STATS_TABLE}(day,payload_json,updated_at) VALUES(%s,%s,%s) ON CONFLICT(day) DO UPDATE SET payload_json=EXCLUDED.payload_json,updated_at=EXCLUDED.updated_at''',(key,payload,datetime.now(timezone.utc).isoformat()))
            return
    ...
    def _bucket(self,day=None):
        ...
        d.setdefault('assets',{});d.setdefault('ai',{});d.setdefault('actions',{});d.setdefault('timing',{'sl_under_5m':0,'sl_5_15m':0,'sl_over_15m':0,'t1_under_5m':0,'t1_5_15m':0,'t1_over_15m':0});return key,d
    @staticmethod
    def _pair(container,key):
        return container.setdefault(key,{'success':0,'failed':0})
    def new_signal(self,action,ai_status='',underlying='UNKNOWN'):
        with self.lock:
            key,d=self._bucket();d['total']+=1;d['unresolved']+=1;d['ai_confirmed' if self._ai_group(ai_status)=='confirm' else 'no_ai_confirmation']+=1;self._pair(d['actions'],action);self._pair(d['assets'],underlying);self._pair(d['ai'],self._ai_group(ai_status));self._save_day(key)
    def resolve(self,action,success,underlying='UNKNOWN',ai_status='',elapsed_seconds=None):
        with self.lock:
            key,d=self._bucket();d['unresolved']=max(0,d['unresolved']-1);d['success' if success else 'failed']+=1;side='buy' if 'BUY' in action.upper() else 'sell';d[f'{side}_{"success" if success else "failed"}']+=1;result='success' if success else 'failed';self._pair(d['actions'],action)[result]+=1;self._pair(d['assets'],underlying)[result]+=1;self._pair(d['ai'],self._ai_group(ai_status))[result]+=1
        ...
        with self.lock:
            ...
            for day in keys:
                ...
                for group in ('assets','ai','actions'):
                    for name,p in src.get(group,{}).items():q=self._pair(out[group],name);q['success']+=int(p.get('success',0));q['failed']+=int(p.get('failed',0))
